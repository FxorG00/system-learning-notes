...
> 版本：2026-09-02，保留 Tensor layout/aliasing 认知闸门
...
## 3. 资料索引：正文优先，官方文档用于查证

首次阅读不需要先完成本节链接。先用下方 API 最小例子完成 metadata predictions；Round1 review 后，再用 Tensor Views、Storage 与 API docs 校准 observed behavior。

### 3.1 Tensor API 与环境查证
...
### 3.2 metadata prediction 完成后查对象模型
