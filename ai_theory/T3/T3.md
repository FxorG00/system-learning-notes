...
[\ldots,M,K]\ @\ [\ldots,K,N]
\longrightarrow
[\operatorname{broadcast}(\ldots),M,N]
$$
...
5. 矩阵转置及 $(AB)^T=B^TA^T$
6. 向量内积
7. 向量的 $L_1$ 范数与 $L_2$ 范数

恢复到能回答下面四题就停止复习：

1. 为什么 $A\in\mathbb R^{M\times K}$ 与 $B\in\mathbb R^{K\times N}$ 可乘，结果为什么属于 $\mathbb R^{M\times N}$？
2. 若先对 $\mathbf x$ 做 $A$，再做 $B$，组合矩阵为什么写成 $BA$？
3. 两个长度为 $K$ 的实向量做内积，结果是什么类型的对象？
4. 对 $\mathbf v=(3,-4)^T$，$L_1$ norm 与 $L_2$ norm 分别是多少？
...
若 $C=AB$，一个 output element 的数学定义是：
...
```mermaid
flowchart LR
    L[left matrix] --> R1[choose one row]
    R[right matrix] --> C1[choose one column]
    R1 --> D[dot product over inner dimension]
    C1 --> D
    D --> O[one output element]
    O --> S[assemble all output positions]
```
...
所以“先 $A$、后 $B$”的组合矩阵写作 $BA$。对应 NumPy code 是 `B @ A`；程序执行顺序从右向左读。
...
1. 检查 core inner dimensions $K$ 是否相等。
2. 对 left batch 与 right batch 做 broadcasting。
3. 每个 broadcasted batch position 做一次 $[M,K]\ @\ [K,N]$。
4. 拼成 $[\text{broadcasted batch}\ldots,M,N]$。
...
验证 $B(A\mathbf x)$ 与 $(BA)\mathbf x$ allclose。
...
普通 broadcasting 从最右 axis 对齐：相等、其中一个为 $1$、或缺失 axis 才 compatible。

N-D `.T` 会反转全部 axes；只转置每个 batch matrix 时交换最后两个 axes。

先推 shape，再运行 code；reference 证明 values，NumPy 证明 library mapping。
