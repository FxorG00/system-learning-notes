...
> 版本：2026-09-01，按 Week11 理论出口与当前数学基础校准
...
1. [D2L 3.4《softmax 回归》](https://zh.d2l.ai/chapter_linear-networks/softmax-regression.html)：只读 `3.4.1 分类问题` 和 `3.4.4 softmax运算`，看到 softmax 公式即可；先跳过 loss、derivative、training 与 implementation。
2. 吴恩达 [P60《多类别》](https://www.bilibili.com/video/BV1owrpYKEtP?p=60) 与 [P61《Softmax》](https://www.bilibili.com/video/BV1owrpYKEtP?p=61)：建立 multiclass output 与 softmax 概念。
3. 李沐 [09《Softmax 回归 + 损失函数 + 图片分类数据集》](https://www.bilibili.com/video/BV1K64y1Q7wu/)：Round1 前只看“Softmax 回归”，损失函数和从零实现先不看。

Round1 前不要播放吴恩达 P63 和李沐 14 的数值稳定性部分。先让极端输入把问题暴露出来。

### 3.2 Round1 review 后再看

1. 回到 [D2L 3.4《softmax 回归》](https://zh.d2l.ai/chapter_linear-networks/softmax-regression.html) 阅读 `3.4.6 损失函数`，只到 cross entropy 与 one-hot target 的关系。
2. 吴恩达 [P62《带 Softmax 输出的神经网络》](https://www.bilibili.com/video/BV1owrpYKEtP?p=62)：确认 logits、softmax output 与 network output 的位置。
3. 吴恩达 [P63《Softmax 的改进实现》](https://www.bilibili.com/video/BV1owrpYKEtP?p=63)：对照 stable implementation 与 combined loss 的动机。
4. 李沐 [09《Softmax 回归 + 损失函数 + 图片分类数据集》](https://www.bilibili.com/video/BV1K64y1Q7wu/) 的“损失函数”，以及 [14《数值稳定性 + 模型初始化和激活函数》](https://www.bilibili.com/video/BV1u64y1i75a/) 的“数值稳定性”；其余内容今天不展开。
5. [NumPy `exp`](https://numpy.org/doc/stable/reference/generated/numpy.exp.html)、[`isfinite`](https://numpy.org/doc/stable/reference/generated/numpy.isfinite.html) 与 [`errstate`](https://numpy.org/doc/stable/reference/generated/numpy.errstate.html)：作为 executable evidence 的 API reference。
...
`exp` 保留大小顺序，所以 `argmax(logits) == argmax(softmax(logits))`。
...
令 `m=max(z)`、`shifted=z-m`，则：

```text
shifted_i <= 0
max(shifted) == 0
exp(shifted_i) <= 1
至少一个 exp(shifted_i) == 1
```
...
```text
target probability 越接近 1 -> loss 越接近 0
target probability 越小      -> loss 越大
```
...
target probability 小到 float64 可能直接表示成 `0`，于是 `-log(0) -> inf`。但数学 loss 约为 `2000`，仍是可表示的 finite value。因此要直接在 log domain 中计算。
...
cross_entropy(z,target=t) = LSE(z) - z_t
```

```mermaid
flowchart TD
    A[finite logits z] --> B[m = max(z)]
    B --> C[shifted = z - m]
    C --> D[exp shifted]
    D --> E[sum shifted exponentials]
    E --> F[LSE = m + log sum]
    F --> G[log_probability_i = z_i - LSE]
    G --> H[loss = LSE - target_logit]
```
