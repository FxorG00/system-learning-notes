...
1. 对 $L=(wx+b)^2$，分别手算 $\frac{\partial L}{\partial w}$ 与 $\frac{\partial L}{\partial b}$。
2. scalar-valued function $L:\mathbb R^n\to\mathbb R$ 对 vector $\boldsymbol\theta$ 求 gradient，结果 shape 是什么？
3. vector-valued function $f:\mathbb R^n\to\mathbb R^m$ 的 Jacobian shape 是什么？
4. 为什么 $\boldsymbol\theta\leftarrow\boldsymbol\theta-\eta\nabla L$ 使用负号？
...
对一个 scalar parameter $\theta$，central difference 为：
...
再用两边 function value 的差除以两点距离 $2\varepsilon$，近似当前位置的 derivative。
...
forward：计算 intermediate value 与 scalar loss
analytical path：使用手推公式计算 w、b 的 gradients
...
$$
z=5,\qquad L=25,\qquad
\frac{\partial L}{\partial w}=30,\qquad
...
## 9. backward 为什么从 $\frac{\partial L}{\partial L}=1$ 开始
...
$$
L=z^2,\qquad
\frac{\partial L}{\partial z}=2z=10,\qquad
\frac{\partial L}{\partial L}=1
$$

所以传到 $z$ 的 gradient 为 $1\times10=10$。
...
$$
z=m+b,\qquad
\frac{\partial z}{\partial m}=1,\qquad
\frac{\partial z}{\partial b}=1
$$

$$
\frac{\partial L}{\partial m}
=\frac{\partial L}{\partial z}\frac{\partial z}{\partial m}=10,
\qquad
\frac{\partial L}{\partial b}
=\frac{\partial L}{\partial z}\frac{\partial z}{\partial b}=10
$$

经过 multiplication：

对 multiply operation：

$$
m=wx,\qquad
\frac{\partial m}{\partial w}=x=3,\qquad
\frac{\partial m}{\partial x}=w=2
$$

$$
\frac{\partial L}{\partial w}
=\frac{\partial L}{\partial m}\frac{\partial m}{\partial w}
=10\times3=30
$$

$$
\frac{\partial L}{\partial x}
=\frac{\partial L}{\partial m}\frac{\partial m}{\partial x}
=10\times2=20
$$

完整执行链：

forward：$w,x\to m=6\to z=5\to L=25$。
...
\to
\left(\frac{\partial L}{\partial m},\frac{\partial L}{\partial b}\right)=(10,10)
\to
\left(\frac{\partial L}{\partial w},\frac{\partial L}{\partial x}\right)=(30,20)
$$

Round1 只要求返回 `w,b` gradients，因为它们模拟 parameters；`x` 当前视作固定 input。数学上 $\frac{\partial L}{\partial x}$ 仍然存在。
...
两个 input paths 分别贡献 $w$，因此：

$$
\frac{\partial L}{\partial w}=w+w=2w
$$
...
$$
\nabla_{\boldsymbol\theta}L\in\mathbb R^N
$$
...
=(3.6,-4.8,1.2)^T,
\qquad
\frac{\partial L}{\partial b}=2z=2.4
$$
...
[ ] scalar analytical gradients 与运行前手算 oracle 一致
...
backward 从 $\frac{\partial L}{\partial L}=1$ 开始，并使用：

$$
\text{input contribution}
=\text{upstream gradient}\times\text{local derivative}
$$

同一 parameter 经多条路径影响 loss 时，各路径 gradient contributions 必须相加。

scalar loss 对 parameter array 的 gradient 与 parameter 具有相同 shape。

finite difference 通过 parameter 扰动和两次 forward 独立近似 gradient；它是 correctness oracle，不是训练方法。

gradient descent 沿 negative gradient 更新，完整训练闭环留到 T7。
