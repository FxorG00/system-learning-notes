...
> 版本：2026-09-20，结合 T1~T3 实际进度定向润色版
...
诊断标准：能手算 $(wx+b-y)^2$ 对 $w,b$ 的偏导，就直接继续，不重听完整微积分课程。
...
### 2.1 derivative、partial derivative 与 gradient

- `derivative`：导数；一元函数的局部变化率。
- `partial derivative`：偏导数；多变量函数中，只考察一个变量变化时的导数。
- `gradient`：梯度；scalar function 对全部 parameters 的 partial derivatives 按 parameter shape 组织起来。
...
`computation graph` 是计算图：用 nodes 表示 values 或 operations，用 directed dependencies 表示“谁的结果被谁使用”。

它不是图像文件，也不是为了画图而增加的一层包装；它把复合计算拆成局部 operations，让每个 operation 只处理自己的 forward relation 与 local derivative。

### 2.3 local derivative 与 upstream gradient

- `local derivative`：局部导数，只描述当前 operation 的 output 怎样依赖 input。
- `upstream gradient`：上游梯度，表示最终 loss 已经怎样依赖当前 operation 的 output。
...
### 2.4 finite difference、perturbation 与 epsilon

`finite difference` 是有限差分：在相距有限距离的采样点上观察 function output 的变化，近似目标位置的 derivative。

`perturbation` 是扰动：对一个 input coordinate 增加或减少一个很小的量。

$\epsilon$ 读作 epsilon；在这里是 perturbation size，不是所有数值算法共用的固定常数。

## 3. 用一个 scalar loss 建立完整 forward graph
...
- 开始前如果连 chain rule 手算都卡住：回自己的微积分笔记，只复习第 1 节列出的标题。
