...
8. expectation
9. variance，以及 $\operatorname{Var}(X)=\mathbb E[X^2]-\mathbb E[X]^2$
10. Bernoulli distribution
11. law of large numbers 的结论

恢复到能回答下面五题即可：

1. $P(A\mid B)$ 与 $P(B\mid A)$ 为什么通常不同？
2. $X\sim\operatorname{Bernoulli}(p)$ 的 expectation 与 variance 是什么？
3. 给 `values=[-1,0,2]`、`probabilities=[0.2,0.5,0.3]`，手算 expectation 与 variance。
4. PMF 的一个点可以直接表示 probability；PDF 的一个点为什么通常不能？
5. independence 与 mutually exclusive 有什么区别？
...
| conditional probability | 条件概率 | 已知事件 $B$ 发生时事件 $A$ 的 probability，记作 $P(A\mid B)$ |
...
-> outcome 1 的 probability 由传入 parameter 决定
...
代码参数 `probability` 对应 $p$，`sample_count` 对应 $N$。
...
$P(A\mid B)$ 表示已知 $B$ 发生时，$A$ 发生的 probability；$P(B\mid A)$ 则交换了 conditioning direction。
...
$$
P(\mathrm{flag}\mid\mathrm{spam})=\frac{80}{100}=0.8
$$

$$
P(\mathrm{spam}\mid\mathrm{flag})
=\frac{80}{80+45}=0.64
$$
...
PDF $f_X(x)$ 是 density，区间 probability 为：

$$
P(a\le X\le b)=\int_a^b f_X(x)\,dx
$$

对 continuous random variable，单点 probability $P(X=x)$ 通常为 $0$。
...
$$
\mathbb E[X]=(-1)\times0.2+0\times0.5+2\times0.3=0.4
$$

$$
\mathbb E[X^2]=1\times0.2+0\times0.5+4\times0.3=1.4
$$
...
[ ] 能用固定邮件例子区分两个相反的 conditional directions
...
distribution 描述 probability rule；sample 是一次具体结果。

random variable 把 outcome 映射成 value，expectation/variance 是 distribution 的理论性质。

sample mean/variance 来自有限 observations，通常接近但不会精确等于理论值。

$P(A\mid B)$ 与 $P(B\mid A)$ conditioning direction 不同。

categorical probability vector 描述所有 categories；sampling 一次只选择一个 category。

category index 只是标识时，expected index 可能没有业务意义。

fixed seed 提供 reproducibility，不把随机样本变成理论分布本身。
