...
1. 在纸上画出 $\mathbf e_1$、$\mathbf e_2$、$A\mathbf e_1$、$A\mathbf e_2$，并说明为什么后二者能确定二维 linear transformation。
2. 手算 $A=\begin{bmatrix}2&-1\\1&3\end{bmatrix}$ 作用于 $\mathbf x=(3,-2)^T$。
3. 写出 $T(\alpha\mathbf u+\beta\mathbf v)$ 的 linearity relation。
4. 说明 linear transformation 与加了 nonzero translation 的 affine transformation 有何区别。
...
| linear transformation | 线性变换 | $\mathbf y=A\mathbf x$ 的数学视角 |
| affine transformation | 仿射变换 | $\mathbf y=A\mathbf x+\mathbf b$ 且 $\mathbf b\ne\mathbf{0}$ |
...
| $A\mathbf e_1$ | `A[:, 0]` |
| $A\mathbf e_2$ | `A[:, 1]` |
| $\mathbf x$ 的 coordinates | `x[0]`、`x[1]` |
| $A$ 作用于 $\mathbf x$ | `A @ x` |
...
```mermaid
flowchart LR
    X[input coordinates] --> B1[scale standard basis 1]
    X --> B2[scale standard basis 2]
    B1 --> C1[scale transformed basis 1]
    B2 --> C2[scale transformed basis 2]
...
满足 $f(\mathbf{0})=\mathbf{0}$，可以是 linear transformation。
...
计算 `A @ zero + bias`，验证结果等于 `bias` 且不等于 zero。用这一条 executable evidence 解释为什么非零 bias 破坏 $f(\mathbf{0})=\mathbf{0}$。
...
[ ] 能解释带 nonzero bias 的 Linear layer 严格说是 affine
...
$A$ 的 columns 是 standard basis vectors 经 transformation 后的位置，而且：
...
$\mathbf y=W\mathbf x$ 是 linear；$\mathbf y=W\mathbf x+\mathbf b$ 在 $\mathbf b\ne\mathbf{0}$ 时严格说是 affine。
