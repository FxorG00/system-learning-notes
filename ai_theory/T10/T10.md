...
- [PyTorch Modules note](https://docs.pytorch.org/docs/stable/notes/modules.html)：Round1 前只读 defining modules 与 parameters 第一层。
...
继续遵守 `ai_theory/ENVIRONMENT.md`。T10 使用与 T9 相同的 Python/PyTorch baseline，但目录有自己的 `.venv`：
...
### 7.4 Round1 observable contract
...
完成后先停，把 code、actual keys/shapes、对象分类和问题写进 T10 note，再让我验收。

R1 正式通过后，我会按你的真实类结构、变量名、输出和问题定向润色 R2/R3；不要先把后面的 checkpoint/mode control 组合答案读完再复刻。

## 8. Reading Gate：R1 通过后再继续

下面开始解释 Module registration、四类对象、mode control 与 checkpoint round trip。R1 尚未通过时先停。
...
## 20. Round2：把你的 R1 映射到 Module tree

R1 通过后，在 note 中用自己的变量名回答：
...
若你的 R1 类结构符合 contract，不要求为了教程变量名重写。

## 21. Round3：完成 Inference 与 Checkpoint 闭环
...
Block 2: Round1 TinyClassifier + forward
Block 3: R1 introspection/assertions + review
