...
> 版本：2026-09-24，统一理论讲解标准复检版
> 当前真实进度：T1~T3 已通过，下一模块是 T4；T14 仍是提前稿
...
这段计算解决的实际问题是：**同一种局部 pattern 可能出现在 image 的不同位置，我们希望用同一组 weights 去检查所有位置。**

局部读取对应 locality（局部性）；同一个 $K$ 到处复用对应 weight sharing（权重共享）。把这类 operator 逐层组合起来的 network，才叫 CNN（convolutional neural network，卷积神经网络）。

## 2. 从单通道例子扩展到真实 Tensor
...
例如 `[2,3,8,8]` 表示两张 samples，每张有三个 channels，每个 channel 的 spatial map 是 `8×8`。Network 某层产生的 channel map 常叫 feature map（特征图）；它是 activation Tensor 的一个切片，不是另存的一张图片。

Kernel（卷积核）是沿 spatial dimensions 滑动的 learnable weights。`groups=1` 时，一个 output channel 会读取所有 input channels 的 local patches，分别乘权后累加，而不是只看一张 input channel。

如果 input channels 为 $C_{\mathrm{in}}$、output channels 为 $C_{\mathrm{out}}$、kernel size 为 $K_H\times K_W$，weight shape 是：

$$
[C_{\mathrm{out}},C_{\mathrm{in}},K_H,K_W]
$$

这条 shape 先于 API：每个 output channel 都需要一整组覆盖所有 input channels 的 local weights。

## 3. Stride、padding 与 pooling 都在改变“怎样读局部区域”

Stride（步幅）决定 window 每次移动多少格。Padding（填充）让 boundary 外的若干位置按零参与计算；它改变 operator 的读取规则，不是在 caller input 中永久插入一圈 values。

Pooling（池化）也读取 local window，但执行固定 reduction，例如 maximum。它通常用来减少 spatial size，没有 convolution 那样的 learnable kernel parameters。

这些词不用提前背成词典。先记住物理动作：

```text
convolution：local weighted sum，weights 可学习且到处复用
stride：window 移动间隔
padding：boundary 外按规定补值
pooling：local fixed reduction
```

## 4. 本课最后为什么还要 residual path

深层 network 若每层都只接上一层 output，optimization 会越来越困难。Residual connection（残差连接）增加一条 shortcut（捷径分支），让 block 学习对 input 的修正：

$$
y=\operatorname{ReLU}\bigl(F(x)+x\bigr)
$$

$F(x)$ 是 main branch，$x$ 是 identity shortcut（保持 value 的恒等映射）。

相加要求两条 path 的 shape 完全一致。Round1 先处理 shape 不变的 identity case；当 channels 或 spatial size 改变时，再从这个失败推出 projection shortcut（投影捷径），而不是现在提前背它的实现。
...
```text
读懂 NCHW Conv2d input/output
推导 kernel/stride/padding 后的 output shape
解释 channel accumulation 与 weight shape
观察 pooling
独立完成 teaching ResidualBlock forward
分开统计 parameters、buffers 与 selected activations
```
...
```text
linear combination
matrix multiplication 中一个 output element 的乘加
Tensor shape/axis
element count × bytes per element
```

PyTorch 侧复用 T9/T10 的 Tensor metadata、`nn.Module`、parameter/buffer、`state_dict` 与 inference boundary。

范围固定为 `dilation=1, groups=1`。完整 convolution backward、CNN training、ResNet-18、CUDA convolution algorithms、depthwise/groups 等留给真实岗位或项目需求。

## 6. `Conv2d` 的 public API 怎样读
...
- [李沐 20：填充和步幅](https://www.bilibili.com/video/BV1Th411U7UN/)：重点对应第 7 节 output shape。
- [李沐 21：多输入多输出通道](https://www.bilibili.com/video/BV1MB4y1F7of/)：重点对应第 8 节 channel accumulation。
- [李沐 22：池化层](https://www.bilibili.com/video/BV1EV411j7nX/)：重点对应本节，不展开训练结果。

已经能独立推 shape 时，不必为“完整播放”重复消耗时间。

## 10. Residual connection 为什么需要两条 path
...
## 13. Round1 的最小 executable evidence
...
## 15. Shape-changing shortcut：两条 path 怎样重新对齐
...
## 16. 与 torchvision `BasicBlock` 对照
...
## 18. Parameter、buffer 与 activation memory 怎样分账

### 18.1 Tensor 自身的 payload bytes
...
### 18.3 Selected activations 不是 peak memory
...
## 19. 用 forward hook 观察 shape，不改 model output contract
...
4. 使用第 18 节 helper，分别打印 parameter bytes、buffer bytes 和 selected activation bytes。
