...
### shared_ptr 解决 std::function 要求内部 target 可复制

```text
然后，如果说你硬是要用 STD function的话，那我们的解决方案就是说，用这个 STD function，把它内部的这个 target，它是一个 lambda。以后我们看到 lambda，就是把它看成一个 closure object。然后你的这个 closure object，它里面 capture 了一个 shared_ptr。这样子你这个 shared_ptr，它是可以 copy 的，并且你内部的这个 function 内部的这个 callable，它确确实实是可以复制的，因为它 capture 的这个 shared_ptr，它是可以复制的。所以你这个 function 就成立的。然后你这个 shared_ptr，你就指向 make_share 时新创建的在堆上的对象。
```
