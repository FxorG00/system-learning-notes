...
2026-09-13 Day4 R1 第二次短复检：destructor 已增加 `start_flag_` 条件，未调用 `start()` 或 ADD 前失败时不再错误执行 DEL；`backlog` 修复仍保留。唯一剩余 blocker 是 started 路径直接调用可能抛 `std::system_error` 的 `EventLoop::remove_channel()`，没有在 destructor 内阻止 exception 外逃。由于 destructor 隐式 `noexcept`，DEL failure 会触发 `std::terminate`，仍不满足 R1 的 non-throwing cleanup contract。下一次只检查这一点；已完成的正常 probe、drain 与 flags 不重复返工。
...
用户还需要能够直接参考的英文 diagnostic text，不应为了每个异常再自行查询翻译。以后 error-contract table 在确有多种 failure conditions 时增加“建议英文 message/context”列：`invalid_argument`/`logic_error` 使用简短的 `Class::method + violated expectation`，例如 `"Connection::send requires a started connection"`；`system_error` 的第三个参数只写 syscall/operation context，例如 `"recv"`、`"fcntl(F_GETFL)"`、`"epoll_ctl MOD"`，具体系统错误由 error code 自动追加。建议 message 用于人类诊断，不默认成为逐字匹配的 test contract；tests 优先核对 exception type、error code 与触发条件。标准库或 callable 自身抛出的 exception 保留原 `what()`，不要捕获后武断改写成统一 message。

### Public method 必须先解释最直白的功能，再讲内部状态
...
## 2026-09-15：Week10 Day6 教程生成

已生成 `week10/day6/day6.md`，严格承接正式通过的 Day5 真实实现。当前 baseline 是：`EventLoop::poll_once` 使用 user-space `epoll_event[1024]` batch 和 `std::map<int, Channel*>` registry；`Channel::handle_event` 依次 dispatch read/write/error callbacks；`Connection` 用 `close_flag_` 保证 close request 幂等；server owner 使用 `unordered_map<int, unique_ptr<Connection>>` 与 `pending_close`，在整个 `poll_once` 返回后 `erase` owner。Day6 不重写 echo 功能，主问题是 callback 中 request close 后，same ready record、same epoll batch、Channel/Connection object lifetime 与 fd reuse 怎样保持一致。

教程保持主线 daily 的三 Part 与 R1/R2/R3，不受 AI Theory 规则影响。R1 唯一产出是 `tests/reactor_lifetime_probe.cpp`：人工设置 `EPOLLIN | EPOLLOUT`，先验证 normal combined dispatch，再让 read callback 设置 close-request state，确定性记录同一 `handle_event` 是否出现 post-close write callback。R1 不实际 destroy Connection、不制造 UAF、不修改生产代码；先写 prediction，再取得 trace，避免在认知墙前泄露 hardening 答案。CMake target 可直接使用，因为 build glue 不是当天设计重点。

R2 在阅读闸门后沿用户真实调用栈讲清：callback 内立即 `connections.erase(fd)` 会销毁仍在执行栈中的 Connection/Channel；close request、object destruction 和 fd close 是三个时刻；当前 deferred cleanup 的安全边界是整个 `poll_once` batch 返回；already-returned user-space event array 不会因 earlier cleanup 自动消失；fd integer reuse 可能造成 old record 与 new object identity confusion。`map_[fd]` 对 missing key 会插入 null `Channel*`，因此 dispatch lookup 必须用 `find`。`epoll_event.data.u64` generation token 只讲 identity 作用，当前 single-thread V1 选择 strict batch-end cleanup，不强制实现 token 或 shared_ptr。

R3 初始唯一升级路线基于当前实现：close-requested Connection handlers 不再开始新的 recv/send/getsockopt work，但 `peer_write_closed_flag_` 不能误当 close guard，因为 EOF with pending output 仍要 drain；EventLoop dispatch 改成 non-inserting lookup；`remove_channel` 即使 `epoll_ctl DEL` 失败，也必须先清除仍指向当前 Channel 的 registry entry，再按 context 报告已保存的 errno，否则 destructor 吞 exception 后 map 会留下 dangling `Channel*`；保留 callback request -> batch end -> owner erase -> unregister -> destroy -> UniqueFd close。复用 Day5 tests/clients，只新增 deterministic lifetime evidence、CTest registration、repeated same-process connections 与 ASan/UBSan。TSan、generation counter、shared ownership、README 和重复客户端均不作为 Day6 默认任务。R1 正式通过后，必须以用户当时磁盘上的 probe、daily 增补、trace 与设计为 edit base，再逐节定向润色 R2/R3。

本次技术校对依据 Linux man-pages：`epoll_ctl` 保存 application 提供的 `epoll_event.data`，`epoll_wait` 将该 data 与 ready bits 写入 caller event array；`epoll(7)` 明确提醒 event cache 中 earlier processing 关闭对象后，later cached record 仍需 removed marking/cleanup policy；`close` 后 fd integer 可被复用。可复用教程经验：lifetime 日不能只罗列“不要 delete this”，应先拆清 callback invocation、one record、whole batch、registration、fd 与 C++ object 六个不同边界，再给完整主语明确的调用链。R1 可用 synthetic ready mask 隔离 network timing，只要它确实确定性建立当天的核心状态转换，就不是 dirty work。

## 2026-09-15：Week10 Day6 Round1 正式通过
...
## 2026-09-23：新理论第一次建立直觉的讲解顺序

用户明确偏好“目标明确 -> 具体直觉 -> 本质推导 -> 公式拆解 -> 最后命名术语”的首次讲解方式，不喜欢先铺多层背景、集中罗列陌生术语，再到很后面才说明今天要解决什么。以后无论系统主线还是 AI Theory，只要首次引入一个新的理论或机制，第一层就必须回答：
...
3. 从这个例子抽出本质关系或物理动作
4. 逐项推导公式、状态或数据流
5. 再说明“这个对象/动作在领域里叫 xxx”
6. 补正式术语、边界、反例和工程含义
7. 自然连接到下一个真正需要的概念
```

术语不是删除，而是后置到读者已经看见其必要性的位置。定义优先使用“英文原词 + 中文含义 + 当前物理动作/绝对规则”，例如先让用户理解“根据 Header 的数字精确拿若干 bytes”，再命名为 `Content-Length framing`；先理解“预测与真实答案差多少的统一分数”，再命名为 `cost function`。不要用脱离主线的术语百科作为第一次入口。

公式讲解必须从问题本身推出，不能只给最终形式。至少说明：

```text
每个 symbol 指向哪个真实对象
当前一项在计算什么
为什么需要平方、求和、平均、负号或缩放
公式的 output 怎样影响下一步动作
代码里的 arrays/shapes/loops 与公式哪一部分对应
```

当公式需要求导或 shape 变换时，按局部步骤拆解，明确每一步使用的规则和中间结果。用户数学基础扎实，不需要重讲完整微积分或线性代数课程；但新的领域映射必须讲清，不能用“显然”“由此可得”跳过真正的新连接。

语言与版式要求：

- 结论先行，删除“接下来我将解释”“有以下几点需要注意”等空起手式。
- 把用户当作聪明、只是第一次接触该业务的同事；不使用焦虑式训诫或反复提醒“不要犯错”。
- 一段通常不超过 3~4 句话；并列规则使用列表或表格；关键结论用少量加粗作为扫描锚点。
- 一层先讲完整，不用十层铺垫；正常主线闭环后再讲设计权衡、异常分支和相邻术语。
- 比喻只负责建立第一层直觉，随后必须回到准确公式、对象与数据流，不能让比喻代替机制。
- **先把“它是什么”写成清晰、加粗的正向结论。** 不在后面堆叠多条“它不是什么”；只有一个相邻概念真的会导致实现错误时，才用一句话做必要区分。
...
## 2026-09-23：长篇机器学习自学讲义的语言标杆

用户提供 `ML/ML.md` 的早期对话式讲解，明确喜欢的不是其原有知识密度，而是它的语气、用词、断句、句子长度、章节编排和自然连接方式。以后编写完整机器学习课程、数学到 AI 的映射讲义或类似长篇理论材料时，采用以下风格：
...
正文应像一位老师连续讲课，而不是工程规范手册：

- 使用“现在的问题是……”“别急着背公式，我们把它拆开”等自然过渡，但删除对话残留式的“你想先看哪个”“准备好了吗”。
- 句子以短句和中等长度句为主；一段只推进一个核心意思，公式前后都用自然语言衔接。
- 比喻负责第一次建立直觉，紧接着必须回到准确公式、shape、数据流、数值边界或计算复杂度。
- 英文术语放在对应对象已经出现的位置命名，不在开头堆术语表；重要术语保留英文原词和中文含义。
- 每节需要内容深度，不能只给科普结论；核心算法至少交付 objective、关键公式、执行步骤、一个例子或图，以及适用边界。
- 图像必须服务当前推导，并写在它所解释的段落附近；不能把图片集中堆在章节末尾，也不能只有图没有读图说明。
- 长课程仍保持完整编号和清晰标题，避免对话复制导致缺课号、同一内容重复多遍或章节突然跳转。
- 章节连接要回答“上一节还缺什么，所以为什么进入下一节”，而不是机械写“接下来进入第 N 课”。

2026-09-23 用户进一步明确：新概念不能用“先报术语，再列定义、职责、边界”的词典式写法，也不能扩写成百科全书式历史介绍。第一次讲一个概念时，应让它从当前问题里自然长出来：
...
例如讲 neuron 时，不先写“neuron 的定义与职责”，而是先说明手工组合特征会爆炸，因此需要能学习小判断的单元；再推出“加权输入 + bias + activation”的结构，最后命名为 neuron。历史来源只用一句有助于理解的说明，不展开百科式沿革。整段要像老师面对面把问题讲通，不能像 API reference、术语词典或组件规格书。这个因果顺序用于真正的新理论；不机械给每个已知小词套五步模板，也不因此增加重复篇幅。

`ML/ML.md` 的职责是传统机器学习全景与自学讲义；`ai_theory/T*.md` 继续承担 NumPy/PyTorch/Transformer 的代码 gate；系统主线 daily 继续遵守 Week11 Day3 标杆、三 Part、R1 独立实现和 executable evidence。长篇 ML 文风不能覆盖或削弱主线 daily 的工程教学规则。

2026-09-23 用户复检后补充两条必须执行的可读性规则：

1. **英文术语不能先于解释冲进正文。** 一个真正的新英文术语首次出现时，紧跟括号补充“中文含义 + 当前上下文中的作用”，例如 `logits（未经概率归一化的原始分类分数）`、`all-reduce（让多个 worker 汇总并共享归约结果的通信操作）`。后续重复使用可以直接写英文或中文，不机械重复括号；公式符号、代码变量、URL 和已经在前文定义的词不算新术语。
2. **图片不能突然插入。** 每张教学图片必须紧跟一段 `> **读图：** ...` 图注，至少说明图中主要对象/坐标/颜色是什么、读者应观察哪一处、它与上一段结论有什么联系。图片必须放在所解释的概念附近，图注不能只把 alt text 换一种说法。
